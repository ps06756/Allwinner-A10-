...
#include <sys/arm/allwinner/a10_dma.h> 
...
struct a10_dma_softc { 
	...
	struct a10_dma_controller a10_dma_cnt ;  
...
struct a10_dma_controller { 
	struct a10_dma_softc* sc ; 
	struct mutex a10_dma_controller_mtx ; 
	struct a10_dma_channel ddma_channels[DDMA] ; 
	struct a10_dma_channel ndma_channels[NDMA] ; 
...
static void a10_dma_detach(device_t) ; 
...
static int a10_dma_probe(device_t dev) 
{
	if(!ofw_bus_status_okay(dev))
		return (ENXIO) ; 
	if(!ofw_bus_is_compatible("allwinner,sun4i-a10-dma"))
		return (ENXIO) ; 
...
static int a10_dma_attach(device_t dev)
{
	...
	if(a10_dma_mem_resource == NULL) { 
		device_printf(dev, "Cannot allocate memory resource !\n") ; 
		a10_dma_release_resources(sc) ; 
		return (ENXIO) ; 
	}
	...
	if(a10_dma_irq_resource == NULL) { 
		device_printf(dev, "Cannot allocate irq resource!\n") ; 
		a10_dma_release_resources(sc) ; 
		return (ENXIO) ; 
	}

	if(bus_setup_intr(sc->a10_dma_dev, sc->a10_dma_irq_resource, INT_MPSAFE | INTR_TYPE_MISC, NULL,a10_dma_intr, sc, &sc->a10_dma_intrhand)) { 
...
static void a10_dma_detach(device_t dev)
{
...
static void a10_dma_release_resources(device_t dev)
{
	...
	if(sc->a10_dma_mem_resource != NULL) 
		bus_release_resources(dev, SYS_RES_MEMORY,sc->a10_dma_mem_rid, sc->a10_dma_mem_resource) ; 

	if(sc->a10_irq_mem_resource != NULL) { 
		bus_teardown_intr(dev, sc->a10_dma_mem_resource, sc->a10_dma_intrhand) ; 
		bus_release_resources(dev, SYS_RES_IRQ, sc->a10_dma_irq_rid, sc->a10_dma_irq_resource) ; 
	} 
}
